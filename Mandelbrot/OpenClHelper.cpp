...
cl::Program createProgram(const std::string& file)
{
	...
	std::string testString(std::istreambuf_iterator<char>(testFile), (std::istreambuf_iterator<char>()));

	cl::Program::Sources sources(1, std::make_pair(testString.c_str(), testString.length() + 1));
